...
int main(int argc, char *argv[])
{
    ...
    int i, j, k, m, spot_range, individual_range, halo_count = -1, halo_number = 0, halo_index = 0, pixel_Rvir_factor = 0, line_number = 0, skip_line = 0, particle_num = 0, pixel_R_size = 0, pixel_Rvir = 0;
    ...
    for (i = 12; i < halo_index; i++) { // for (i = 0; i < halo_index; i++)
        ...
        // Skipping the first 10 lines.
        part *parts_comov = malloc((line_number) * sizeof *parts_comov);
        part *parts_ang = malloc((line_number) * sizeof *parts_ang);
        float *part_ang_x = malloc((line_number) * sizeof(float));
        float *part_ang_y = malloc((line_number) * sizeof(float));
        float *part_ang_z = malloc((line_number) * sizeof(float));
        ...
        while(fscanf(halo_individual, "%f %f %f", &parts_comov[particle_num].x, &parts_comov[particle_num].y, &parts_comov[particle_num].z) == 3) {
            ...
            particle_num++;
            if (particle_num == line_number)   // To avoid the last line #OK
                break;
        }
        ...
        for (j = 0; j < particle_num; j++) { // (j = 0; j < particle_num; j++)
